...
from seco.GRU import *
from seco.HCN import HCN
from seco.AGCN import Model as AGCN
...
parser.add_argument('--schedule', default=[50, 70, 85], nargs='*', type=int,
...
def load_moco_encoder_q(model,pretrained):
        ...
        if os.path.isfile(pretrained):
            ...
            print("message",msg)
            assert set(msg.missing_keys) == {'fc.weight', 'fc.bias', 'fc_intra.weight', 'fc_intra.bias', 'fc_order.weight', 'fc_order.bias', 'fc_order_classifier.weight', 'fc_order_classifier.bias'}
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif args.finetune_dataset== 'ucla':
        opts = options.opts_ucla()
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # if data_size[1] != 64:
        #     images = images.view(-1,64,data_size[-1])
        if args.gpu is not None:
        ...
        # compute output
        output = model(images,knn_eval=True)
        # # 对每三个output取平均，因为每三个是一个video
        # out_size = output.size()
        # output = output.view(data_size[0],-1,out_size[1])
        # output = torch.mean(output, dim=1).view(-1,out_size[1])
...
def sanity_check_encoder_q(state_dict, pretrained_weights):
    ...
    for k in list(state_dict.keys()):
        ...
        if 'fc.weight' in k or 'fc.bias' in k or 'fc_intra.weight' in k or 'fc_intra.bias' in k or 'fc_order.weight' in k or 'fc_order.bias' in k or 'fc_order_classifier.weight' in k or 'fc_order_classifier.bias' in k:
            continue
