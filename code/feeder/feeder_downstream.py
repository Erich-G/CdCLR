...
class Feeder(torch.utils.data.Dataset):
    ...
    def get_aug(self,clip,number_of_frames):
        data_numpy = clip
        ...
        if  self.input_representation == "seq-based":
             ...
             input_data = input_data.reshape(-1,60).astype('float32')
             return input_data
    ...
    def __getitem__(self, index):
        ...
        # sample_num = 4
        # frames = torch.cat([torch.from_numpy(self.get_aug(clip=data_numpy,number_of_frames=number_of_frames)) for i in range(sample_num)], dim=0)        
        frames = self.get_aug(clip=data_numpy,number_of_frames=number_of_frames)    
        #input 
        return frames,label
