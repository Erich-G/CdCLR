...
def joint_courruption(input_data):                                                                     
    ...
    if flip_prob < 0.5:
        ...
        joint_indicies = np.random.choice(20, 10,replace=False)
...
def temporal_cropresize(input_data,num_of_frames,l_ratio,output_size):
    ...
    # Temporal crop
    min_crop_length = 64 #64
 
    scale = np.random.rand(1)*(l_ratio[1]-l_ratio[0])+l_ratio[0] # 尺度大小
    temporal_crop_length = np.minimum(np.maximum(int(np.floor(num_of_frames*scale)),min_crop_length),num_of_frames) # 裁剪后的作物大小

    start = np.random.randint(0,num_of_frames-temporal_crop_length+1) # 选取一个起始位置

    temporal_context = input_data[:,start:start+temporal_crop_length, :, :] # 截取片段
